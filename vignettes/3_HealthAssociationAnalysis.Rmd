...
df$acc_quintiles <- cut(df$acc.overall.avg, breaks = quantile(df$acc.overall.avg, probs = seq(0, 1, by = 0.2), na.rm = TRUE)) # cut at quintiles
```
...
```{r table}
table1::table1(~ sex + broadAgeGroup + ethnicity + smoking | acc_quintiles, data=df)
```
...
# Format date data
death$date_of_death <- as.Date(df$date_of_death, format = "%d/%m/%Y")
...
df$fu_time <- as.numeric(difftime(df$follow_up, df$end_accel_wear))
```
...
```{r}
data_for_plot <- summary(cox_model)$conf.int
...
data_for_plot <- data_for_plot[(ncoef - 4):ncoef, ]

ref_row <-  data.frame("var_name" = "Reference: Lowest fifth of acceleration", "HR" =1,  "ignore" = "ignore", "lower_CI"= 1, "upper_CI"  =1)
data_for_plot <- rbind(ref_row, data_for_plot)

ggplot(data_for_plot, aes(x = "HR", y = "var_name")) + geom_point() + geom_errorbar(aes(xmin = lower_CI, xmax = upper_CI), width = 0) + geom_vline(xintercept = 1) + scales::scale_x_continuous(trans = "log")
```
...
- you might want to look into model diagnostics to understand if assumptions, such as the [proportional hazards assumption](https://thomaselove.github.io/432-notes/cox-regression-models-for-survival-data-example-1.html) (see section 23.2.5 and 23.2.6).
...
- you could also refine the outcome definition by refining the set of ICD-10 codes you use to identify
...
- you might want to consider some **sensitivity analyses**. For example, [could the association be driven by participants who have reduced their activity due to poor health](https://pubmed.ncbi.nlm.nih.gov/32472927/)? How could you assess that? 
