...
```{r table}
table1::table1(~ sex + broadAgeGroup | acc_quintiles, data=dAll)
```
...
dAll$died <- 0
dAll$died[dAll$eid %in% death$eid ] <- 1 # Add an indicator for death
...
died <- (dAll$died == 1)
dAll$censoring[died] <- pmin(dAll$censoring[died], as.Date(dAll$date_of_death[died], format = "%d/%m/%Y"))
...
had_ihd <- dAll$ischaemic.heart.disease.incident == 1 # make indicator variable
dAll$follow_up[had_ihd] <- pmin(as.Date(dAll$ischaemic.heart.disease[had_ihd], format = "%Y-%m-%d"), dAll$censoring[had_ihd])

# Note event status at censoring (again, care taken as there are some instances of entries in the data after censoring)
dAll$IHD_at_exit <- 0
dAll$IHD_at_exit[had_ihd & (as.Date(dAll$ischaemic.heart.disease, format = "%Y-%m-%d") == dAll$follow_up)] <- 1
